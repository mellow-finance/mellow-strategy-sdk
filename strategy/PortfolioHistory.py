...
class PortfolioHistory:
    ...
    def portfolio_stats(self):
        ...
        for name, pos in self.portfolio.positions.items():
            ...
            if hasattr(pos, 'impermanent_loss_to_x'):
                pos_fees_to_x = pd.DataFrame([pos.history_il_to_x], index=[f'il_to_x_{name}']).T
                dfs.append(pos_fees_to_x)

            if hasattr(pos, 'impermanent_loss_to_y'):
                pos_fees_to_y = pd.DataFrame([pos.history_il_to_y], index=[f'il_to_y_{name}']).T
                dfs.append(pos_fees_to_y)
    ...
    def draw_portfolio_to_y(self, portfolio_df):
        ...
        fig.add_trace(
            go.Scatter(
                x=portfolio_df.index,
                y=portfolio_df['total_volume_to_y'] + portfolio_df['total_fees_to_y'],
                name="Volume to Y",
            ),
            secondary_y=False)
    ...
    def draw_performance_y(self, portfolio_df):
        ...
        fig.add_trace(
            go.Scatter(
                x=portfolio_df.index,
                y=portfolio_df['performance_to_y_yearly'],
