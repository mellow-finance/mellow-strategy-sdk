...
class DownloadFromS3:
    ...
    def __init__(self, data_dir: str, bucket_name: str = "mellow-public-data") -> None:
...
class RawDataUniV3:
    ...
    def __init__(self, pool: Pool, data_dir: str, reload_data: bool = False) -> None:
        self.pool = pool
        self.data_dir = data_dir
    ...
    def load_mints(self) -> pl.DataFrame:
        ...
        assert (
            self.pool._address in df_mints_raw["pool"].unique()
        ), f"Pool {self.pool._address} is not available yet."
        ...
        df_prep = (
            df_mints.select(
                [
                    ...
                    pl.col("block_number"),
                    pl.col("log_index"),
                    ((pl.col("block_time") * 1e3 + pl.col("log_index")) * 1e3)
                    .cast(pl.Datetime)
                    .alias("timestamp"),
                    ...
                    (pl.col("amount") / 10**self.pool.l_decimals_diff).alias(
                        "liquidity"
                    ),
                ]
            )
            .with_column(pl.col("timestamp").dt.truncate("1d").alias("date"))
            .with_column(pl.Series(name="event", values=["mint"]))
    ...
    def load_burns(self) -> pl.DataFrame:
        ...
        assert (
            self.pool._address in df_burns_raw["pool"].unique()
        ), f"Pool {self.pool._address} is not available yet."
        ...
        df_prep = (
            ...
            .with_column(pl.col("timestamp").dt.truncate("1d").alias("date"))
            .filter((pl.col("amount0") + pl.col("amount1")) > 1e-6)
            .with_column(pl.Series(name="event", values=["burn"]))
            .sort(by=["block_number", "log_index"])
        )
        return df_prep
    ...
    def load_swaps(self) -> pl.DataFrame:
        ...
        assert (
            self.pool._address in df_swaps_raw["pool"].unique()
        ), f"Pool {self.pool._address} is not available yet."
        ...
        df_prep = (
            df_swaps.select(
                [
                    ...
                    pl.col("amount0") / 10**self.pool.token0.decimals,
                    pl.col("amount1") / 10**self.pool.token1.decimals,
                    (pl.col("liquidity") / 10**self.pool.l_decimals_diff).alias(
                        "liquidity"
                    ),
            ...
            .with_column(
                ...
                .apply(
                    lambda x: float(
                        (Decimal(x) * Decimal(x))
                        / (Decimal(2**192) / Decimal(10**self.pool.decimals_diff))
                    )
                )
                .alias("price")
            )
            .with_columns(
                [
                    pl.col("price")
                    .shift_and_fill(1, pl.col("price").first())
                    .alias("price_before"),
                    pl.col("price")
                    .shift_and_fill(-1, pl.col("price").last())
                    .alias("price_next"),
...
class SyntheticData:
    ...
    def generate_data(self) -> PoolDataUniV3:
        ...
        price_log_returns = np.random.normal(
            loc=self.mu, scale=self.sigma, size=len(timestamps)
        )
        ...
        df = pl.DataFrame([timestamps, prices]).with_column(
        ...
        df = df.with_column(
            (np.trunc(np.log(pl.col("price")) / np.log(1.0001))).alias("tick")
        )
...
class DownloaderBinanceData:
    ...
    def get(self) -> pd.DataFrame:
        ...
        # num of nano sec [us]
        us_num = (
            int(self.interval[0:-1]) * map_dict[self.interval[-1]] * 60 * 1000 * 1000
        )
        ...
        file_name = (
            f"{self.pair_name}_{self.interval}_{self.start_date}_{self.end_date}.csv"
        )
